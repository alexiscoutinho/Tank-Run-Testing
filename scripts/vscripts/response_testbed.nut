...
{
	local duration
	local ent = contexttoworld ? Entities.First() : speaker

	foreach ( context in contextData )
	{
		duration = 0
		if ( context.len() == 3 )
			duration = context[2]
		...
		ent.SetContext( context[0], context[1].tostring(), duration )
	}
...
function rr_ProcessRules( rulesarray )
{
	...
	{
		// preprocessing
		if ( typeof rule.criteria[0] != "array" )
			rule.criteria = [ rule.criteria ]
		if ( typeof rule.responses != "array" )
			rule.responses = [ rule.responses ]

		if ( "applycontext" in rule )
		{
			if ( typeof rule.applycontext[0] != "array" )
				rule.applycontext = [ rule.applycontext ]

			foreach ( r in rule.responses )
			{
				if ( "applycontext" in r )
				{
					if ( typeof r.applycontext[0] != "array" )
						r.applycontext = [ r.applycontext ]
					r.applycontext.extend( rule.applycontext )
				}
				else
					r.applycontext <- rule.applycontext
			}
		}
		if ( "applycontexttoworld" in rule )
		{
			foreach ( r in rule.responses )
			{
				if ( !("applycontexttoworld" in r) )
					r.applycontexttoworld <- rule.applycontexttoworld
			}
		}
